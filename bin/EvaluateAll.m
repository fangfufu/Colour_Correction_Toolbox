...
foldInd = crossvalind('KFold',size(XYZ,1), 4); %#ok<NODEF>
...
% i = i + 1;
% % 
% [ cielabE ] = EvalCCRGBXYZ(RGB, XYZ, wp, @GenCCEnsemble, @ApplyCCEnsemble, foldInd);
% t(i,:) = GenerateTableRow(cielabE, 'Ensemble method');
% i = i + 1;

[ cielabE ] = EvalCCRGBXYZ(RGB, XYZ, wp, @GenCCHPP, @ApplyCCHPP, foldInd);
